...
var NEWS = {
  ...
  spectroton: [
    {
      date: '2026-09-23',
      de: 'Spectroton zählt jetzt Besuche anonym mit GoatCounter, ohne Cookies — wie die übrigen Werkzeuge der Werkstatt; der Footer sagt es offen. Das Zählskript wird mit dem Werkzeug ausgeliefert statt von einem fremden Dienst nachgeladen, und die Farben eines geteilten Links stehen im Anker der Adresse, der nie mitgesendet wird.',
      en: 'Spectroton now counts visits anonymously with GoatCounter, no cookies — like the other tools in the workshop, and the footer says so. The counting script ships with the tool instead of being loaded from someone else’s service, and the colours of a shared link sit in the address fragment, which is never sent along.',
      es: 'Spectroton cuenta ahora las visitas de forma anónima con GoatCounter, sin cookies, como las demás herramientas del taller; el pie de página lo indica abiertamente. El script de recuento se entrega con la herramienta en lugar de cargarse desde un servicio ajeno, y los colores de un enlace compartido van en el ancla de la dirección, que nunca se envía.',
      fr: 'Spectroton compte désormais les visites de façon anonyme avec GoatCounter, sans cookies — comme les autres outils de l’atelier, et le pied de page le dit clairement. Le script de comptage est livré avec l’outil au lieu d’être chargé depuis un service tiers, et les couleurs d’un lien partagé se trouvent dans l’ancre de l’adresse, qui n’est jamais transmise.',
      it: 'Spectroton ora conta le visite in forma anonima con GoatCounter, senza cookie, come gli altri strumenti dell’officina; il piè di pagina lo dice apertamente. Lo script di conteggio viene fornito insieme allo strumento invece di essere caricato da un servizio altrui, e i colori di un link condiviso stanno nell’ancora dell’indirizzo, che non viene mai inviata.',
      nl: 'Spectroton telt bezoeken nu anoniem met GoatCounter, zonder cookies — net als de andere gereedschappen van de werkplaats, en de voettekst zegt het openlijk. Het telscript wordt met het gereedschap meegeleverd in plaats van bij een externe dienst te worden opgehaald, en de kleuren van een gedeelde link staan in het anker van het adres, dat nooit wordt meegestuurd.',
      pl: 'Spectroton liczy teraz odwiedziny anonimowo za pomocą GoatCounter, bez plików cookie — tak jak pozostałe narzędzia warsztatu, a stopka mówi o tym otwarcie. Skrypt zliczający jest dostarczany razem z narzędziem, zamiast być pobierany z cudzej usługi, a kolory udostępnionego linku są zapisane w kotwicy adresu, która nigdy nie jest wysyłana.',
      pt: 'O Spectroton conta agora as visitas de forma anónima com o GoatCounter, sem cookies — como as restantes ferramentas da oficina, e o rodapé di-lo abertamente. O script de contagem é entregue com a ferramenta em vez de ser carregado de um serviço alheio, e as cores de um link partilhado ficam na âncora do endereço, que nunca é enviada.',
      tr: 'Spectroton artık ziyaretleri GoatCounter ile anonim ve çerezsiz sayıyor — atölyenin diğer araçları gibi; alt bilgi bunu açıkça belirtiyor. Sayım betiği başka bir hizmetten yüklenmek yerine araçla birlikte geliyor ve paylaşılan bir bağlantının renkleri adresin # işaretinden sonraki kısmında duruyor; bu kısım asla gönderilmiyor.',
      ru: 'Spectroton теперь анонимно считает посещения с помощью GoatCounter, без файлов cookie — как и остальные инструменты мастерской; в подвале страницы об этом сказано прямо. Скрипт подсчёта поставляется вместе с инструментом, а не загружается с чужого сервиса, а цвета из общей ссылки хранятся в части адреса после #, которая никогда не отправляется.',
      hi: 'Spectroton अब GoatCounter से बिना कुकीज़ के गुमनाम रूप से विज़िट गिनता है — वर्कशॉप के बाकी औज़ारों की तरह; फ़ुटर में यह साफ़ लिखा है। गिनती की स्क्रिप्ट किसी दूसरी सेवा से लोड होने के बजाय औज़ार के साथ ही आती है, और साझा लिंक के रंग पते में # के बाद वाले हिस्से में रहते हैं, जो कभी भेजा नहीं जाता।',
      zh: 'Spectroton 现在用 GoatCounter 匿名统计访问量，不使用 Cookie——与工坊里的其他工具一样，页脚中也写明了这一点。统计脚本随工具一起提供，而不是从别人的服务加载；共享链接中的颜色保存在地址 # 之后的部分，这部分从不会被发送。',
      ja: 'Spectroton は GoatCounter で訪問数を匿名で数えるようになりました。Cookie は使いません — 工房のほかの道具と同じで、フッターにもはっきり書いてあります。集計スクリプトは他人のサービスから読み込むのではなく、道具と一緒に配信されます。共有リンクの色はアドレスの # 以降の部分に入っていて、そこが送られることはありません。',
      ko: 'Spectroton은 이제 GoatCounter로 방문 수를 익명으로 셉니다. 쿠키는 쓰지 않습니다 — 작업실의 다른 도구들과 같으며, 푸터에도 분명히 적혀 있습니다. 집계 스크립트는 다른 서비스에서 불러오지 않고 도구와 함께 제공되며, 공유 링크의 색은 주소의 # 뒤 부분에 들어 있어 절대 전송되지 않습니다.'
    },
    {
      date: '2026-09-22',
      de: 'Behoben: Stufen, deren Buntheit für sRGB gekappt werden musste, trugen in jeder Sprache das deutsche Schild „eng“, und weder dieses noch „P3“ erklärte sich. Jetzt steht dort ein Wort in der Sprache der Oberfläche. Die Erklärung erscheint beim Darüberfahren mit der Maus, auf Handy und Tablet als Legende unter der Rampe, und Vorleseprogramme lesen sie mit. Auf schmalen Bildschirmen rückt das Schild unter den HEX-Wert, damit die Kontrastspalten bündig bleiben.',
      en: 'Fixed: steps whose chroma had to be clipped to sRGB carried the German label “eng” in every language, and neither it nor “P3” explained itself. The label is now a word in the interface language. The explanation appears when hovering with the mouse, on phones and tablets as a legend below the ramp, and screen readers read it out too. On narrow screens the label moves below the HEX value so the contrast columns stay aligned.',
      es: 'Corregido: los pasos cuya saturación había que recortar a sRGB llevaban en todos los idiomas la etiqueta alemana «eng», y ni esta ni «P3» se explicaban. Ahora la etiqueta es una palabra en el idioma de la interfaz. La explicación aparece al pasar el ratón por encima, en móviles y tabletas como leyenda bajo la rampa, y los lectores de pantalla también la leen. En pantallas estrechas la etiqueta pasa debajo del valor HEX para que las columnas de contraste sigan alineadas.',
      fr: 'Corrigé : les paliers dont la saturation devait être écrêtée pour tenir dans sRGB portaient dans toutes les langues l’étiquette allemande « eng », et ni elle ni « P3 » ne s’expliquaient. L’étiquette est désormais un mot dans la langue de l’interface. L’explication s’affiche au survol de la souris, sur téléphone et tablette sous forme de légende sous la rampe, et les lecteurs d’écran la lisent aussi. Sur les écrans étroits, l’étiquette passe sous la valeur HEX pour que les colonnes de contraste restent alignées.',
      it: 'Risolto: i gradini la cui saturazione andava limitata a sRGB portavano in tutte le lingue l’etichetta tedesca «eng», e né questa né «P3» si spiegavano. Ora l’etichetta è una parola nella lingua dell’interfaccia. La spiegazione compare passandoci sopra con il mouse, su telefono e tablet come legenda sotto la scala, e anche i lettori di schermo la leggono. Sugli schermi stretti l’etichetta scende sotto il valore HEX, così le colonne del contrasto restano allineate.',
      nl: 'Opgelost: stappen waarvan de verzadiging voor sRGB moest worden afgekapt, droegen in elke taal het Duitse label “eng”, en noch dat label noch “P3” legde zichzelf uit. Nu staat er een woord in de taal van de interface. De uitleg verschijnt met de muis erboven, op telefoon en tablet als legenda onder de schaal, en schermlezers lezen die ook voor. Op smalle schermen schuift het label onder de HEX-waarde, zodat de contrastkolommen recht blijven.',
      pl: 'Naprawione: stopnie, których nasycenie trzeba było przyciąć do sRGB, nosiły we wszystkich językach niemiecką etykietę „eng”, a ani ona, ani „P3” niczego nie wyjaśniały. Teraz etykieta jest słowem w języku interfejsu. Wyjaśnienie pojawia się po najechaniu myszą, na telefonie i tablecie jako legenda pod skalą, a czytniki ekranu też je odczytują. Na wąskich ekranach etykieta przesuwa się pod wartość HEX, żeby kolumny kontrastu pozostały wyrównane.',
      pt: 'Corrigido: os níveis cuja intensidade da cor tinha de ser limitada ao sRGB traziam em todas as línguas a etiqueta alemã «eng», e nem esta nem «P3» se explicavam. Agora a etiqueta é uma palavra na língua da interface. A explicação aparece ao passar o rato por cima, no telemóvel e no tablet como legenda por baixo da rampa, e os leitores de ecrã também a leem. Em ecrãs estreitos a etiqueta passa para baixo do valor HEX, para que as colunas de contraste fiquem alinhadas.',
      tr: 'Düzeltildi: Renk yoğunluğu sRGB’ye sığacak şekilde kırpılması gereken adımlar her dilde Almanca “eng” etiketini taşıyordu; ne bu etiket ne de “P3” ne anlama geldiğini açıklıyordu. Artık etiket arayüz dilinde bir kelime. Açıklama fareyle üzerine gelince, telefon ve tablette ise rampanın altında lejant olarak beliriyor; ekran okuyucular da onu okuyor. Dar ekranlarda etiket HEX değerinin altına iniyor, böylece kontrast sütunları hizalı kalıyor.',
      ru: 'Исправлено: ступени, насыщенность которых пришлось обрезать до sRGB, на всех языках носили немецкую метку «eng», и ни она, ни «P3» ничего не объясняли. Теперь метка — слово на языке интерфейса. Пояснение появляется при наведении мыши, на телефоне и планшете — в виде легенды под шкалой, а программы экранного доступа тоже его зачитывают. На узких экранах метка переходит под значение HEX, чтобы столбцы контраста оставались ровными.',
      hi: 'ठीक किया गया: जिन सीढ़ियों की संतृप्ति sRGB में समाने के लिए सीमित करनी पड़ी, उन पर हर भाषा में जर्मन लेबल “eng” लगा रहता था, और न वह, न “P3” अपना मतलब बताता था। अब लेबल इंटरफ़ेस की भाषा का एक शब्द है। व्याख्या माउस ऊपर ले जाने पर दिखती है, फ़ोन और टैबलेट पर शृंखला के नीचे संकेत-सूची के रूप में, और स्क्रीन रीडर भी उसे पढ़ते हैं। संकरी स्क्रीन पर लेबल HEX मान के नीचे आ जाता है, ताकि कंट्रास्ट वाले कॉलम सीध में रहें।',
      zh: '已修复：彩度需要裁切至 sRGB 的色阶，在所有语言中都带着德语标签“eng”，而且它和“P3”都没有任何说明。现在标签是界面语言中的一个词。说明在鼠标悬停时显示，在手机和平板上则以图例形式显示在色阶下方，屏幕阅读器也会朗读。在窄屏上，标签移到 HEX 值下方，让对比度各列保持对齐。',
      ja: '修正: sRGB に収めるために鮮やかさを抑えた段には、どの言語でもドイツ語のラベル「eng」が付いていて、それも「P3」も意味の説明がありませんでした。いまはラベルが画面の言語の単語になりました。説明はマウスを重ねると表示され、スマートフォンやタブレットではランプの下に凡例として表示されます。スクリーンリーダーも読み上げます。幅の狭い画面では、コントラストの列がそろったままになるよう、ラベルが HEX 値の下に移ります。',
      ko: '수정: sRGB에 맞추려고 채도를 줄여야 했던 단계에는 어느 언어에서나 독일어 단어 “eng” 표시가 붙어 있었고, 이것도 “P3”도 뜻을 설명하지 않았습니다. 이제 표시는 인터페이스 언어의 낱말로 나옵니다. 설명은 마우스를 올리면 뜨고, 휴대폰과 태블릿에서는 색 단계 표 아래에 범례로 나오며, 화면 낭독기도 이 설명을 읽어 줍니다. 좁은 화면에서는 대비 열이 가지런히 유지되도록 표시가 HEX 값 아래로 내려갑니다.'
    },
    {
      date: '2026-09-14',
